...
import ru.startnet.utils.pgdiff.schema.PgColumn;
import ru.startnet.utils.pgdiff.schema.PgColumnPrivilege;
import ru.startnet.utils.pgdiff.schema.PgColumnUtils;
import ru.startnet.utils.pgdiff.schema.PgInheritedColumn;
import ru.startnet.utils.pgdiff.schema.PgRelationPrivilege;
import ru.startnet.utils.pgdiff.schema.PgSchema;
import ru.startnet.utils.pgdiff.schema.PgTable;
...
public class PgDiffTables {
    ...
    public static void alterTables(final PrintWriter writer,
        ...
        for (final PgTable newTable : newSchema.getTables()) {
            if (oldSchema == null
                    ...
                    || newTable.getName().toLowerCase().contains("flyway_schema_history")) {
                continue;
            }
    ...
    private static void checkTablespace(final PrintWriter writer,
        ...
        if (oldTable.getTablespace() == null && newTable.getTablespace() == null
                || oldTable.getTablespace() != null && oldTable.getTablespace().equals(newTable.getTablespace())
                || newTable.getName().toLowerCase().contains("flyway_schema_history")) {
            return;
        }
    ...
    public static void createTables(final PrintWriter writer,
        ...
        for (final PgTable table : newSchema.getTables()) {
            if ((oldSchema == null
                ...
                && !table.getName().toLowerCase().contains("flyway_schema_history")) {
                searchPathHelper.outputSearchPath(writer);
                writer.println();
